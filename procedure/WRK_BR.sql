DELIMITER $$
DROP PROCEDURE IF EXISTS `WRK_BR`$$
...
BEGIN
 ...
 DECLARE cur CURSOR FOR SELECT wb.PERSONTYPEID, wb.AGENTID, wb.EVENTPARAMS, wb.ACTIONTYPEID, wb.ACTIONTEMPLATE, wb.ACTIONPARAMS FROM wrk_bizrule wb WHERE wb.AGENCYID = iFirmid and wb.IDEVENT = EVTYPE AND wb.ISWRK = 1 order by wb.ORDERPOS;
